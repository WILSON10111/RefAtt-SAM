...
class ATTNSAMImageEncoder(SAMImageEncodeWrapper):

    def __init__(
            self, ori_sam, rs_token: torch.Tensor
    ):
        super(ATTNSAMImageEncoder, self).__init__(ori_sam=ori_sam, fix=True)
        ...
        self.rs_token_down_proj = nn.Sequential(
            *[Adapter(in_features=rs_token.size(-1), mlp_ratio=0.125, add_last_layer=False)
        ...
        self.shared_up_proj = nn.Linear(
            in_features=int(rs_token.size(-1) * 0.125),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rs_feature = torch.cat(
            [self.shared_up_proj(down_proj(self.rs_token)).unsqueeze(-1) for down_proj in self.rs_token_down_proj],
            dim=-1
        )
        prompt = self.prompt_generator.get_prompt(handcrafted_feature, embedding_feature, rs_feature=rs_feature)
