...
from .misc import generate_prompts_from_mask
...
class BaseSegDataset(Dataset):
    ...
    def __init__(
        ...
        for config_dict in dataset_config_list:
            for key, value in config_dict.items():
                corrected_path = value['image_path'].replace('\\', '/')
                corrected_path = os.path.normpath(corrected_path)  # Normalize the path
                config_dict[key]['image_path'] = corrected_path
                corrected_path = value['mask_path'].replace('\\', '/')
                corrected_path = os.path.normpath(corrected_path)  # Normalize the path
                config_dict[key]['mask_path'] = corrected_path
    ...
    def __getitem__(self, index):
        ...
        image_path = self.idx2img_gt_path[index_name]['image_path']
        ...
        return dict(
            images=image, gt_masks=gt_mask, index_name=index_name, image_path=image_path
        )
        ...
        while len(batch) != 0:
            ...
            if ele_dict is not None:
                for key in ele_dict.keys():
                    ...
                    if key == 'image_path':
                        image_paths.append(ele_dict[key])
            # remove the redundant data for memory safety
            batch.remove(ele_dict)
        batch_dict['image_path'] = image_paths
