...
from .modeling import SAMImageEncodeWrapper, SAMPromptEncodeWrapper,ATTNSAMImageEncoder
from .modeling import MaskDecoder_ATTN
...
class BaseATTNSAM(nn.Module):
    mask_threshold: float = 0.0
    image_format: str = "RGB"

    def __init__(self, model_type: str):
        super(BaseATTNSAM, self).__init__()
        ...
        self.mask_decoder = MaskDecoder_ATTN(
    ...
    def preprocess1(self, x: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        masks_hq,_,masks_sam = self.mask_decoder(
    ...
    def infer(
        ...
        masks_pred = self.forward(
            imgs=None, image_embeddings=self.img_features, 
            ...
            attn_sim=attn_sim, target_embedding=target_embedding
        )
...
class ATTNSAM(BaseATTNSAM):
    def __init__(self, model_type: str):
        super(ATTNSAM, self).__init__(model_type=model_type)
        self.image_encoder = ATTNSAMImageEncoder(ori_sam=self.ori_sam, hq_token=self.mask_decoder.hf_token.weight)
