...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    将输入张量填充为从截断正态分布中抽取的值。这些值实际上是从正态分布 :math:\mathcal{N}(\text{mean}, \text{std}^2) 中抽取的，其中超出范围 :math:[a, b] 的值会被重新抽取，直到它们落在指定范围内。用于生成随机值的方法在 :math:a \leq \text{mean} \leq b 的情况下效果最佳。
    参数：
...
class PromptGenerator(nn.Module):
    ...
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
        ...
        self.shared_mlp = nn.Linear(self.embed_dim // self.scale_factor,
                                    self.embed_dim)
